...
class PaginationModel extends Model
{
	...
	{
		$query = 'SELECT u.ci, u.name, u.surname, email, password, 
		address, personal_phone, local_phone, birth_date, gender, `group`, 
		admission_date, r.ci as r_ci, r.name as r_name, r.surname as r_surname 
		FROM users u INNER JOIN extra_info e ON e.student = u.ci INNER JOIN representant r ON 
		r.ci = e.representant WHERE `group` = "Primaria" OR `group` = "Preescolar" 
		ORDER BY name DESC LIMIT '.$start.', '.$items;
